...
SettingHandler::SettingHandler(std::string filename) {
	...
	if (setting_file) {
		...
		throw std::runtime_error("Can not open setting file: " + filename + "; It does not exist or you do not have premssion to read it.");
	}
...
SimulationSetting SettingHandler::getSimualtionSetting(int simulation_index) const{
	...
	std::string file_to_save = file_name_patten; // not to overwrite pattern
//	std::cerr << "file_name_patten " << file_name_patten << std::endl;

	for(unsigned int parameter_index = 0; parameter_index < parameters_in_order.size(); parameter_index++){
		// std::cerr	<< "parameters_in_order " << parameters_in_order[parameter_index] << '\t'
		// 		<< "parameters_numbers " << parameters_numbers[parameter_index] << std::endl;
...
//	std::cerr << " Seed of simulation " << simulation_index << " is set to " << seed[simulation_index] << "\n";
	...
	// cerr << "File to save: " << file_to_save << std::endl;
...
void SettingHandler::printParameters() const{
	int checker = 0;
	std::cerr << std::setw(15) << "loci"
			<< std::setw(15) << "selected_loci"
			<< std::setw(15) << "chromosomes"
			<< std::setw(15) << "deme_size"
	...
	for(int val_index = 0; val_index < number_of_simulations; val_index++){
		...
		std::cerr << std::setw(15);
		if(val_index == 0){ std::cerr << replicates;}
		std::cerr << std::setw(15);
...
void SettingHandler::printWorld() const{
	std::cerr << "##### WORLD SETTING #####\n"
	<< "Dimension: " << dimension << std::endl;
	if(dimension != 0){
		std::cerr << "COLUMNS: " << left_right_demes << std::endl
		<< "ROWS: " << up_down_demes << std::endl
		<< "UD DOWN BORDER: " << type_of_updown_edges << std::endl
		<< "LEFT RIGHT BORDER: " << type_of_leftright_edges << std::endl;
	}
	return;
}
...
void SettingHandler::printParameterCounts() const{
	for(unsigned int i = 0; i < parameters_in_order.size(); i++){
		std::cerr << std::setw(15) << parameters_in_order[i] << ' ';
	}
	std::cerr << std::endl;
	for(unsigned int i = 0; i < parameters_numbers.size(); i++){
		std::cerr << std::setw(15) << parameters_numbers[i] << ' ';
	}
	std::cerr << std::endl;
	return;
}
...
int SettingHandler::parseSetting(std::ifstream& myfile){
	double value;
	std::vector<double> paravec;
	std::string switcher;
	std::string line, parameter, number;
	replicates = 1;
	...
	if (myfile.is_open()){
		while ( getline (myfile,line) ){
			...
			for(unsigned int i = 0;i < line.size();i++){
				...
				if(parameter.substr(0,16) == "NAMEofOUTPUTfile" or parameter.substr(0,16) == "TYPEofOUTPUTfile"){
					if(line[i] == '.' or line[i] == '~' or line[i] == '*' or line[i] == '/' or line[i] == '\\'){
						throw std::runtime_error("Symbols '.' '*' '~' and '/' are not allowed in any parameter value.");
					}
					if(!isspace(line[i])){
						number += line[i];
					}
			...
			if(!parameter.empty()){
				if(paravec.empty()){
//					std::string (file name, file type, world) or value
...
// 					if single value was read
						if(number.empty()){
							//cerr << "Warning, failed to read value of parameter " << parameter << std::endl;
							number.clear();
							parameter.clear();
							paravec.clear();
							continue;
...
int SettingHandler::parseWorldDefinition(std::string& line){
	int switcher = 0, n = 0;
	std::string type, number;
	int stdHeight = 0, stdWidth = -1, stdDim = 0;
	std::string stdLR, stdUD;
	for(unsigned int i = 0;i < line.size();i++){
		if(line[i] == '-'){
			switcher = 1;
		}
...
// 						continue;
// 					}
					throw std::runtime_error("ERROR: the " + type + " world description is not defined (yet?)");
				}
			}
		}
		if(switcher == 2){
		...
		if(switcher == 5){
			if(isdigit(line[i]) and i+1 < line.size()){
				...
				std::cerr << "Error: Unknown pre-defined world " << type << std::endl;
	...
	throw std::runtime_error("ERROR: unknown problem during loading setting file (contact us please)");
}
...
void SettingHandler::parameterSave(std::string& parameter, double value){
	...
	std::cerr << "Warning: unknown parameter: " << parameter << std::endl;
	return;
}

void SettingHandler::parameterSave(std::string& parameter, std::vector<double>& value){
...
//	std::cerr << "REPLICATES: " << replicates << std::endl;
...
template<typename T>
int SettingHandler::printVectorValue(unsigned int index, std::vector<T> val_vec) const{
	if(index < val_vec.size()){
		std::cerr << val_vec[index];
...
void SettingHandler::adjustFileName(std::string& name, char par, int total, int refactorised_index) const{
	if(total > 1){
		if(total > 9){
			name = name + '_' + par + '?' + '!';
...
//		std::cerr << " Error: non specified parameters. \n Expected: \n Found: ";
//
//		for(unsigned int i = 0; i < parameters_in_order.size(); i++){
//			std::cerr << parameters_in_order[i] << " ";
//		}
//		std::cerr << std::endl;
//	}
	...
	for(unsigned int i = 0; i < gen.size(); i++){
		if(gen[i] < 0){
			throw std::runtime_error("Negative NUMBERofGENERATIONS.");
		}
		for(unsigned int j = 0; j < saves.size(); j++){
			...
			for(unsigned int k = 0; k < delay.size(); k++){
				if(saves[j] > (gen[i] - delay[k])){
					throw std::runtime_error("The number of saves (" + std::to_string(saves[j]) +
										") is greater than number of generations after delay (" +
										std::to_string(gen[i] - delay[k]) + ")." );
				}
			}
		}
	}
	...
	if(!file_name_patten.empty()){
		std::vector<std::string> types{"complete", "summary", "hybridIndices", "hybridIndicesJunctions", "blocks", "raspberrypi", "backtrace"};
		...
		if(correct_type > 0){
			throw std::runtime_error("Type of save is invalid: " + type_of_save);
		}
	}
	...
	for(unsigned int i = 0; i < saves.size(); i++){
		if(saves[i] > 0){
			if(file_name_patten.empty()){
				std::cerr << "The name of output file was not specified. The only output will be printed strandard stream.\n";
			}
		}
	}
	...
	if(deme.size() == 0){
		throw std::runtime_error("The DEMEsize was not set.");
	}
//	for(unsigned int i = 0; i < deme.size(); i++){
//		if(deme[i] < 4){
//			std::cerr << "Strange deme size \n"
//		}
//	}

	if(lambda.size() == 0){
		throw std::runtime_error("The LAMBDA ( mean number of crossovers per chromosome per generation) was not set.");
	}
...
//	}
	...
	if(loci.size() == 0){
		throw std::runtime_error("The number of loci per chromosome was not set.");
	}
...
//	}
	...
	for(unsigned int i = 0; i < selected_loci.size(); i++){
		if(selected_loci[i] == 0){
			if(selected_loci.size() > 1){
				throw std::runtime_error("If selected loci parameter is set to 0 it is expected to be only one value only\nIs selected loci are set to 0, all the loci are selected.");
			}
		} else {
			for(unsigned int j = 0; j < loci.size(); j++){
				neutral_block_size = (loci[j] - selected_loci[i]) % (selected_loci[i] - 1);
				if(neutral_block_size != 0){
					std::cerr 	<< "Values of loci (" << loci[j]
							<< ") and selected loci (" << selected_loci[i]
							<< ") are incompatible" << std::endl;
					std::cerr	<< "\t(loci - selected_loci) % (selected_loci - 1)" << std::endl;
					std::cerr	<< "\thas to be equal to 0, but in this case..." << std::endl;
					std::cerr	<< "\t(" << loci[j] << " - " << selected_loci[i] << ") % (" << selected_loci[i] << " - 1) = " << neutral_block_size << std::endl;
					throw std::runtime_error("yeah, it's bit messy!");
				}
			}
		}
	}
	...
	if(sel.size() == 0){
		throw std::runtime_error("The selection pressure was not set.");
	}
...
//	}
	...
	if(beta.size() == 0){
		throw std::runtime_error("The BETA (epistatic interaction between loci) was not set.");
	}
	for(unsigned int i = 0; i < beta.size(); i++){
		if(beta[i] < 0.0001 or beta[i] > 32){
			std::cerr << "Warning: BETA is out of range of reasonable values: " << beta[i] << std::endl;
		}
	}

	if((unsigned)number_of_simulations != seed.size()){
		std::cerr << "The number of seeds is not equal to number of simulation\n";
		std::cerr << "Possible SEED: none (seeds will be random generated using time)\n";
		std::cerr << "               one value (other seeds will be generated using defined seed)\n";
		std::cerr << "               vector of values (has to have length equal to total number of simulations)\n";
		throw std::runtime_error("SEED problem.");
	}

	//cerr << "Checking World definition\n";
	if(dimension == -1){
		throw std::runtime_error("Dimension was not set.");
	}

	if(dimension >= 3){
		throw std::runtime_error("Dimension was not set correctly.");
	}

	if(left_right_demes < 1 and dimension > 0){
		throw std::runtime_error("The width of world was not set.");
	}

	if(up_down_demes < 1 and dimension > 0){
		throw std::runtime_error("The height of world was not set.");
	}

	if(dimension > 0){
		std::vector<std::string> edges {"extending","wrapping","infinite","reflexive"};
		if(type_of_leftright_edges.empty()){
			throw std::runtime_error("The type of left right edges of world was not set.");
		} else {
			correct_type = 1;
			...
			if(correct_type){
				throw std::runtime_error("Type of left-right edges is invalid: " + type_of_leftright_edges);
			}
		}

		if(type_of_updown_edges.empty()){
			if(dimension == 2){
				throw std::runtime_error("The type of up-down edges of 2D world was not set.");
			}
		} else {
			correct_type = 1;
			...
			if(correct_type){
				throw std::runtime_error("Type of UpDown edges is invalid: " + type_of_updown_edges);
			}
		}
	}
...
void SettingHandler::fillDefault(){
	...
	if(selected_loci.empty()){
		std::cerr << "Assuming all loci under selection.\n";
	...
	if(beta.empty()){
		std::cerr << "Assuming no epistatic interaction (BETA = 1).\n";
		beta.push_back(1);
